...
function QuizPage({ addStar }) {
  ...
  return (
    <div className="min-h-screen py-2 md:py-4 px-3 md:px-4">
      <div className="max-w-4xl lg:max-w-5xl mx-auto">
        {/* Compact Navigation Buttons */}
        <div className="mb-2 md:mb-3 flex gap-2 relative z-[100]">
          <button
            onClick={() => navigate('/')}
          ...
          <button
            ...
            className="bg-white/90 backdrop-blur-sm text-gray-700 font-semibold py-1.5 px-3 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 flex items-center gap-1.5 text-sm border border-gray-200 hover:border-gray-300"
          >
            📖 الدرس
          </button>
        </div>
        ...
        <div className="card animate-fadeIn max-w-4xl mx-auto">
          {/* Compact Header */}
          <div className="mb-2 flex items-center justify-between gap-3 pb-2 border-b border-primary/20">
            <div className="flex items-center gap-2">
              <div className="text-2xl">{lesson.icon}</div>
              <div>
                <h1 className="text-base md:text-lg font-black text-gray-800 leading-tight">
                  {lesson.title}
            </h1>
                <div className="flex items-center gap-1.5 text-xs">
                  <span className="bg-blue-100 px-2 py-0.5 rounded-full text-primary font-bold">
                    <span className="number-ltr">{currentQuestion + 1}</span>/<span className="number-ltr">{quiz.length}</span>
                </span>
                  <span className="bg-yellow-100 px-2 py-0.5 rounded-full text-yellow-700 font-bold">
          ...
          <div className="min-h-[300px] md:min-h-[350px] flex flex-col">
          {/* Question */}
            <div className="mb-2 bg-gradient-to-r from-blue-50 to-purple-50 p-3 rounded-lg border-r-2 border-primary">
              <h2 className="text-sm md:text-base lg:text-lg font-bold text-gray-800 leading-relaxed">
              {question.question}
            </h2>
          </div>

            {/* Options */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5 mb-2 flex-grow">
              {question.options.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`${bgColor} border-2 ${borderColor} rounded-lg p-2.5 text-right text-sm md:text-base font-bold transition-all duration-300 shadow-md relative z-10 hover:scale-105 hover:shadow-lg`}
                  >
            ...
            {/* Compact Feedback Space */}
            <div className="h-10 flex items-center justify-center">
              {isAnswered ? (
                <div className={`p-2 rounded-lg text-center text-sm font-bold animate-fadeIn ${
                ...
                <div className="text-gray-400 text-sm">اختر إجابتك...</div>
              )}
            </div>
          </div>

          {/* Compact Navigation Buttons - Directly Above Question Indicators */}
          <div className="mb-2 flex items-center justify-between">
            <div className="flex gap-2">
              <button
                onClick={handlePrevious}
                disabled={currentQuestion === 0}
                className={`px-3 py-1.5 rounded-lg font-bold text-sm transition-all duration-300 relative z-10 flex items-center gap-1 ${
                  currentQuestion === 0
            ...
            {Object.keys(userAnswers).length === quiz.length && (
              <button
                ...
                className="bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1.5 rounded-lg font-bold text-sm transition-all duration-300 shadow-md hover:shadow-lg"
              >
                🎊 عرض النتائج
              </button>
            )}
          ...
          <div className="bg-gray-50 p-2 rounded-lg">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-bold text-gray-700">📋 الأسئلة</span>
